...
export class News extends Component {
  ...
  fetchMoreData = async () => {
    try {
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&sortBy=popularity&category=${this.props.cat}&apiKey=${this.props.apikey}&page=${this.state.page + 1}&pagesize=${this.props.pageSize}`;
  ...
  async componentDidMount() {
    try {
      ...
      console.log("cdm")
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&sortBy=popularity&category=${this.props.cat}&apiKey=${this.props.apikey}&page=1&pagesize=${this.props.pageSize}`;
          ...
          >
            <div className="container my-3">
              <div className="row" style={{ marginTop: "20px", marginBottom: "60px" }}>
                {this.state.internet && this.state.articles.map((e) => {
                  return e.title.includes(this.props.query) && e.url !== "https://removed.com" && <div className="col-md-3" key={e.url}>
                    <Newsitem mode={this.props.mode} title={e.title ? e.title.slice(0, 45).concat("...") : ""} id={e.source.id} description={e.description ? e.description.slice(0, 88).concat("...") : ""} imgurl={e.urlToImage} newsUrl={e.url} date={e.publishedAt} author={e.author} />
                  </div>
                })}
              </div>
            </div>
            {(this.state.internet ? !queryFound : !this.state.internet) && <div className="col-md-12" style={{ height: '426px', marginTop: "150px" }}><div className="card" style={{ maxWidth: '500px', margin: 'auto', boxShadow: '0px 2px 5px rgba(0, 0, 0, 0.1)' }}>
